...
async def endpoint_detail_page(request: Request, endpoint_id: str):
    ...
    for a in agents_inventory:
        ...
        # Get tool names from alert-linked events
        alert_tools = []
        for al in agent_alerts:
            ...
            for eid in eids:
                ...
                if ev.get("tool_name"):
                    alert_tools.append((ev["tool_name"], al))

        # Attach event_count and alert_count to each MCP
        agent_events = [e for e in all_events if e.get("agent_type") == ad["agent_type"]]
        for mcp in ad.get("mcp_servers", []):
            prefix = f"mcp__{mcp['name'].split(' (')[0].replace('-', '_')}__"
            mcp["event_count"] = sum(1 for e in agent_events if (e.get("tool_name") or "").startswith(prefix))
            mcp["alert_count"] = sum(1 for t, _ in alert_tools if t.startswith(prefix))
...
async def agent_detail_page(request: Request, agent_type: str):
    ...
    alerts = await _store.query_alerts(min_severity=0, limit=20)
    agent_alerts = [a for a in alerts if a.get("agent_type") == agent_type]

    agent_dict = agent_inv.to_dict() if agent_inv else {"agent_type": agent_type, "installed": False, "mcp_servers": [], "hooks": [], "extensions": []}

    # Build alert -> tool lookup
    event_by_id = {e.get("id", ""): e for e in all_events}
    alert_tools = []
    for al in agent_alerts:
        try:
    ...
    # Pre-compute activity + alerts per MCP
    for mcp in agent_dict.get("mcp_servers", []):
        prefix = f"mcp__{mcp['name'].split(' (')[0].replace('-', '_')}__"
        mcp["events"] = [e for e in all_events if (e.get("tool_name") or "").startswith(prefix)]
...
async def mcp_detail_page(request: Request, agent_type: str, mcp_name: str):
    ...
    # Normalize: "memory-service" -> "memory_service" for tool prefix matching
    mcp_tool_prefix = f"mcp__{mcp_name.split(' (')[0].replace('-', '_')}__"
    since = datetime.now(timezone.utc) - timedelta(hours=24)
    all_events = await _store.query_events(agent_type=agent_type, since=since, limit=500)
    mcp_events = [e for e in all_events if (e.get("tool_name") or "").startswith(mcp_tool_prefix)]

    # Find alerts related to this MCP's tools using SQL JOIN
    # (avoids the 500-event window limit that caused missed alerts)
...
# =========================================================================
# LEVEL 4: Skill Detail (config + content + invocations)
# =========================================================================

@app.get("/agent/{agent_type}/skill/{skill_name:path}", response_class=HTMLResponse)
async def skill_detail_page(request: Request, agent_type: str, skill_name: str):
    """Drill into a specific skill: config, prompt content, and invocation events."""
    assert _store and _registry
    agents_inventory = discover_all_agents()
    agent_inv = next((a for a in agents_inventory if a.agent_type == agent_type), None)
    ...
    skill = None
    if agent_inv:
        agent_dict = agent_inv.to_dict()
        skill = next((s for s in agent_dict.get("skills", []) if s["name"] == skill_name), None)

    # Read skill file content
    skill_content = ""
    if skill and skill.get("file_path"):
        try:
            skill_content = Path(skill["file_path"]).read_text()
        except Exception:
            pass

    # Find Skill-invocation events (UserPromptSubmit with skill name pattern)
    since = datetime.now(timezone.utc) - timedelta(hours=24)
    all_events = await _store.query_events(agent_type=agent_type, since=since, limit=500)
    skill_events = [e for e in all_events if e.get("action") == "skill_invoke" and e.get("tool_name") == skill_name]

    # Find related alerts
    all_alerts = await _store.query_alerts(limit=200)
    skill_alerts = [a for a in all_alerts if a.get("agent_type") == agent_type and skill_name in (a.get("description") or "")]
    ...
    return templates.TemplateResponse("skill_detail.html", {
        "request": request,
        "agent_type": agent_type,
        "skill": skill or {"name": skill_name, "description": "", "file_path": ""},
...
async def api_test_rule(rule_id: str, request: Request):
    ...
    # Build temporary rule from submitted conditions
    from claude_edr.backend.detection.engine import DetectionRule
    from claude_edr.backend.models.events import Severity

    severity_val = body.get("severity", "MEDIUM")
    severity = Severity[severity_val.upper()] if isinstance(severity_val, str) else Severity(severity_val)
    ...
    # Get recent events and test
    since = datetime.now(timezone.utc) - timedelta(hours=24)
    events = await _store.query_events(since=since, limit=500)

    matches = []
    for e in events:
        # Build a mock EDREvent-like object for field resolution
        class _EventProxy:
            pass
